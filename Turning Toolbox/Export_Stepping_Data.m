...
Heading = {};
for j = 1:12
    Heading = horzcat(Heading,strcat('Step',{' '},num2str(j),{' '},'Foot'));
    Heading = horzcat(Heading,strcat('Step',{' '},num2str(j),{' '},'Onset Latency'));
    Heading = horzcat(Heading,strcat('Step',{' '},num2str(j),{' '},'Placement Time'));
    Heading = horzcat(Heading,strcat('Step',{' '},num2str(j),{' '},'Duration'));
    Heading = horzcat(Heading,strcat('Step',{' '},num2str(j),{' '},'Amplitude'));
    Heading = horzcat(Heading,strcat('Step',{' '},num2str(j),{' '},'Size'));
    Heading = horzcat(Heading,strcat('Step',{' '},num2str(j),{' '},'Velocity'));
    Heading = horzcat(Heading,strcat('Step',{' '},num2str(j),{' '},'Acceleration'));
end
Heading = horzcat(Heading,'Total Steps');
Heading = horzcat(Heading,'Stepping Frequency');
Heading = horzcat(Heading,'Stepping Strategy');
Heading = horzcat(Heading,'Turn Duration');
...
Filename = sprintf(char(strcat(ExpName,{' '},ParticipantID,{' '},'Stepping Data.xlsx')));
warning('off','all');
...
for TrialNum = 1:NumTrials;
    ...
    if TF==0
        DataExport = [];
        for n = 1:SteppingData.Total;
            DataExport = horzcat(DataExport,SteppingData.SortedSteps(n,:)); %#ok<*AGROW>
        end
        
        Add = 12 - SteppingData.Total;
        Blanks = 8*Add;
        AddBlanks = NaN(1,Blanks);
        AddBlanks = num2cell(AddBlanks);
        DataExport = horzcat(DataExport,AddBlanks);
        DataExport = horzcat(DataExport,SteppingData.Total);
        DataExport = horzcat(DataExport,SteppingData.SteppingFrequency);
        DataExport = horzcat(DataExport,SteppingData.Strategy);
        DataExport = horzcat(DataExport,SteppingData.SortedSteps(end,3));
...
[a,b,c] = xlsread(char(strcat(ExpName,{' '},ParticipantID,{' '},'Stepping Data.xlsx'))); %#ok<*ASGLU>
...
save(char(strcat(ExpName,{' '},ParticipantID,{' '},'Stepping Data.mat')));
Folder = pwd;
...
movefile(char(strcat(ExpName,{' '},ParticipantID,{' '},'Stepping Data.mat')),Destination);

disp('Export Stepping Data script complete');
