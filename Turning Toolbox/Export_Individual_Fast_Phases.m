...
for TrialNum = 1:NumTrials;
    ...
    if TF == 0;
        ...
        Data = horzcat(FastPhases.Onset_Position,FastPhases.End_Position,FastPhases.Amplitudes,FastPhases.Velocities,FastPhases.Accelerations);
        ...
        [Max,MaxRow] = max(Sum);
        if MaxRow==1
            Data1 = vertcat(Data1,Data); %#ok<*AGROW>
        elseif MaxRow==2
...
save([char(strcat(ExpName,{' '},ParticipantID,{' '},'Individual Fast Phases'))]); %#ok<*NBRAK>
Folder = pwd;
...
movefile(char(strcat(ExpName,{' '},ParticipantID,{' '},'Individual Fast Phases.mat')),Destination);

disp('Export Individual Fast Phases script complete');
