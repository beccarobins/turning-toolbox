function Plot_Overlayed_Segment_Displacement
...
for TrialNum = TS:NumTrials;
    ...
    if TF == 0;
        ...
        plot(Xaxes,SteppingData.LeadingFoot.Displacement,'k');
        plot(Xaxes,SteppingData.TrailingFoot.Displacement,'k');
        TrialNumber = num2str(TrialNum);
        Title = strcat('Trial Number',{' '},TrialNumber);
        title(strcat('Segment Displacement -',Title));
        legend('Head','Thorax','Pelvis','Lead Foot','Trail Foot','Location','SouthEast');
        xlabel ('Time(s)');
        ylabel ('Displacement(°)');
        set(gcf, 'Visible','off');
        ...
        if TrialNum<=9
            name = char(strcat(ExpName,{' '},ParticipantID,{' 0'},num2str(TrialNum),{' '},'Overlayed Segment Displacement','.tiff'));
        elseif TrialNum>9
            name = char(strcat(ExpName,{' '},ParticipantID,{' '},num2str(TrialNum),{' '},'Overlayed Segment Displacement','.tiff'));
...
beep
msgbox('Plot Overlayed Segment Displacements Script Complete');
clc
